	...
	if bConquest:
		...
		if CyPlayerOld.isAlive():
			...
			if CyCity.isOccupation():
				...
				if CyTeamOld.isHasTech(GC.getInfoTypeForString("TECH_RIFLING")):
...
################################################
					...
					if citysize < 14:
						...
						if citysize > 27:
							...
							rel_ativ = 2
						rel_ativ = 1 + GAME.getSorenRandNum(rel_ativ, "get number between 1 and 3 depending on city size")
					...
					if CyPlayerOld.getPower() > GC.getPlayer(iOwnerNew).getPower():
						...
						if 6 > GAME.getSorenRandNum(10, "Random for less Partisans"):
							nPartisan -= rel_ativ
					else:
						# 40% chance for more partisans
						if 5 < GAME.getSorenRandNum(10, "Random for more Partisans"):
							nPartisan += rel_ativ
					...
					nPartisan = int((nPartisan * 0.01) * (100 - CyCity.plot().calculateCulturePercent(iOwnerNew)))
					if nPartisan  < 0:
						nPartisan = 0
...
def onCombatResult(argsList):
	...
	if CyUnitW.getUnitType() == GC.getInfoTypeForString('UNIT_PARTISAN'):
		...
		if iCombatL == GC.getInfoTypeForString('UNITCOMBAT_SIEGE'):
			...
			captrureChance = 5
		if captrureChance and GAME.getSorenRandNum(100, "Partisan capture unit") <= captrureChance:
