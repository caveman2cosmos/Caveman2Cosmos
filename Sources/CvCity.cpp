...
void CvCity::doOutbreakCheck(PromotionLineTypes eAfflictionLine)
{
	...
	if (iOutbreakRollResult < iChancetoOutbreak)
	{
		...
		AddDLLMessage(getOwner(), true, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_POSITIVE_DINK", MESSAGE_TYPE_INFO, NULL, CvColorInfo::red(), getX(), getY());
...
void CvCity::doOvercomeCheck(PromotionLineTypes eAfflictionLine)
{
	...
	if (iOvercomeRollResult < iChancetoOvercome)
	{
		...
		AddDLLMessage(getOwner(), true, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_POSITIVE_DINK", MESSAGE_TYPE_INFO, NULL, CvColorInfo::green(), getX(), getY());
