...
UnitTypes CvCityAI::AI_bestUnit(int& iBestUnitValue, int iNumSelectableTypes, UnitAITypes* pSelectableTypes, bool bAsync, UnitAITypes* peBestUnitAI, bool bNoRand, bool bNoWeighting, const CvUnitSelectionCriteria* criteria)
{
	...
	if (!bNoWeighting)
	{
		...
		else
		{
			...
			if (iHasMetCount > 0 && (bLandWar || bAssault || !bFinancialTrouble || GET_PLAYER(getOwner()).getFinalUnitUpkeep() == 0))
			{
...
void CvCityAI::AI_doHurry(bool bForce)
{
	...
	for (int iI = 0; iI < GC.getNumHurryInfos(); iI++)
	{
		...
		if (iHurryPopulation < 1)
		{
			...
			&& !GET_PLAYER(getOwner()).AI_isFinancialTrouble() && iHurryAngerLength == 0)
			{
				...
				else if ((bDanger ? 8 : 12)*iHurryGold < GET_PLAYER(getOwner()).getGold())
				{
					bWait = false;
				}
			...
			{
				const int iValuePerTurn = AI_buildingValueThreshold(eProductionBuilding, BUILDINGFOCUS_GOLD | BUILDINGFOCUS_MAINTENANCE | BUILDINGFOCUS_PRODUCTION) / 3;

				if (iValuePerTurn > 0)
				{
					const int64_t iHurryGold = getHurryGold((HurryTypes)iI);
					...
					if (iHurryGold / iValuePerTurn < getProductionTurnsLeft(eProductionBuilding, 1))
					{
						...
						if (iHurryGold < iGoldThreshold)
						{
							if (gCityLogLevel >= 2)
							{
								logBBAI("      City %S hurry gold at %lld < threshold %lld", getName().GetCString(), iHurryGold, iGoldThreshold);
							}
							hurry((HurryTypes)iI);
							return;
						}
		...
		if (eProductionBuilding != NO_BUILDING)
		{
			...
			&& !GET_PLAYER(getOwner()).AI_avoidScience()
		...
		if (eProductionUnitAI != NO_UNITAI)
		{
			...
			&& !AI_isDefended(-2*GET_PLAYER(getOwner()).strengthOfBestUnitAI(DOMAIN_LAND, UNITAI_CITY_DEFENSE))) // XXX check for other team's units?
			{
				iMinTurns = std::min(iMinTurns, 5);
			}
			...
			&& GET_PLAYER(getOwner()).AI_neededWorkers(area()) > 2*area()->getNumAIUnits(getOwner(), UNITAI_WORKER)
		...
		const int iThreshAdjust = (int)GET_PLAYER(getOwner()).getCurrentEra();
...
void CvCityAI::AI_bestPlotBuild(CvPlot* pPlot, int* piBestValue, BuildTypes* peBestBuild, int iFoodPriority, int iProductionPriority, int iCommercePriority, bool bChop, int iHappyAdjust, int iHealthAdjust, int iFoodChange)
{
	...
	const CvImprovementInfo& plotImp = GC.getImprovementInfo(ePlotImp);
	...
	for (int iI = 0; iI < GC.getNumImprovementInfos(); iI++)
	{
		...
		else if (!impX.isActsAsCity() && impX.getVisibilityChange() == 0)
		{
			if (eForcedBuild == NO_BUILD)
			{
				for (int iJ = 0; iJ < GC.getNumBuildInfos(); iJ++)
				{
					...
					{
						int iValue = 10000;
						iValue /= 1 + GC.getBuildInfo(eBuild).getTime();
			...
			if (eBestTempBuild != NO_BUILD)
			{
				...
				if (pPlot->getFeatureType() != NO_FEATURE
				&& GC.getBuildInfo(eBestTempBuild).isFeatureRemove(pPlot->getFeatureType()))
				{
					bIgnoreFeature = true;

					if (eNonObsoleteBonus == NO_BONUS
					&& GC.getFeatureInfo(pPlot->getFeatureType()).getYieldChange(YIELD_PRODUCTION) > 0)
					{
		...
		if (bValid)
		{
			ImprovementTypes eFinalImprovement = GET_TEAM(getTeam()).finalImprovementUpgrade(eImpX);

			if (eFinalImprovement == NO_IMPROVEMENT)
			{
				eFinalImprovement = eImpX;
			}

			ImprovementTypes eCurImprovement = ePlotImp;
			...
			if (iValue >= 0)
			{
				...
				if (ePlotImp == NO_IMPROVEMENT)
				{
					...
					if (eBestTempBuild != NO_BUILD && pPlot->getFeatureType() != NO_FEATURE
					&& GC.getBuildInfo(eBestTempBuild).isFeatureRemove(pPlot->getFeatureType()))
					{
				...
				else
				{
					...
					if (eNonObsoleteBonus == NO_BONUS)
					{
						...
						{
							if (((iFoodPriority < 100) && (aiFinalYields[YIELD_FOOD] >= GC.getFOOD_CONSUMPTION_PER_POPULATION())) || (plotImp.getImprovementPillage() != NO_IMPROVEMENT))
							{
								iValue -= 70;
								iValue *= 2;
								iValue /= 3;
	...
	if (bChop && (eBonus == NO_BONUS) && (pPlot->getFeatureType() != NO_FEATURE) &&
		(ePlotImp == NO_IMPROVEMENT) && !(GET_PLAYER(getOwner()).isOption(PLAYEROPTION_LEAVE_FORESTS)))
	{
		for (int iI = 0; iI < GC.getNumBuildInfos(); iI++)
		{
			BuildTypes eBuild = ((BuildTypes)iI);
	...
	for (int iI = 0; iI < GC.getNumRouteInfos(); iI++)
	{
		...
		if (eRoute != eOldRoute)
		{
			...
			if (ePlotImp != NO_IMPROVEMENT)
			{
				...
				{
					iTempValue += ((plotImp.getRouteYieldChanges(eRoute, YIELD_FOOD)) * 100);
					iTempValue += ((plotImp.getRouteYieldChanges(eRoute, YIELD_PRODUCTION)) * 60);
					iTempValue += ((plotImp.getRouteYieldChanges(eRoute, YIELD_COMMERCE)) * 40);
				}
