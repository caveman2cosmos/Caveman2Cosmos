...
CvViewport::CvViewport(
	CvMap* pMap,
	bool   bIsFullMapContext)
	: m_pMap(pMap), m_transformType(VIEWPORT_TRANSFORM_TYPE_WINDOW), m_inhibitSelection(false), m_bDisplayCityScreen(false), m_bSelectCity(false), m_bAddSelectedCity(false), m_state(VIEWPORT_ACTION_STATE_NONE), m_countdown(0), m_bSwitchInProgress(false), m_eSpoofHiddenGraphics(VIEWPORT_SPOOF_NONE), m_spoofTransitionStartTickCount(-1)
{
	resizeForMap();
