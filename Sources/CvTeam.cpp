...
void CvTeam::setHasTech(TechTypes eTech, bool bNewValue, PlayerTypes ePlayer, bool bFirst, bool bAnnounce)
{
	...
	for (int iI = 0; iI < MAX_PLAYERS; iI++)
	{
		...
		if (player.isAliveAndTeam(getID()))
		{
			...
			// Toffer - Heritage may change commerce output with tech.
			//	Not part of processTech to avoid it applying twice during recalc, would apply in processHeritage first then in processTech again after.
			for (int iJ = 0; iJ < NUM_COMMERCE_TYPES; iJ++)
			{
				player.changeExtraCommerce100((CommerceTypes)iJ, iChange * player.getHeritageCommerceTechChange((CommerceTypes)iJ, eTech));
			}
