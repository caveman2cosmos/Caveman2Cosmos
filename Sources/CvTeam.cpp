...
void CvTeam::processTech(TechTypes eTech, int iChange, bool bAnnounce)
{
	...
	for (int iI = 0; iI < MAX_PLAYERS; iI++)
	{
		...
		{
			playerX.changeFeatureProductionModifier(kTech.getFeatureProductionModifier() * iChange);
			playerX.changeWorkerSpeedModifier(kTech.getWorkerSpeedModifier() * iChange);
			playerX.changeTradeRoutes(kTech.getTradeRoutes() * iChange);
			playerX.changeExtraHealth(kTech.getHealth() * iChange);
			playerX.changeExtraHappiness(kTech.getHappiness() * iChange);
			playerX.changeDistanceMaintenanceModifier(kTech.getDistanceMaintenanceModifier() * iChange);
			playerX.changeNumCitiesMaintenanceModifier(kTech.getNumCitiesMaintenanceModifier() * iChange);
			playerX.changeMaintenanceModifier(kTech.getMaintenanceModifier() * iChange);
			playerX.changeCoastalDistanceMaintenanceModifier(kTech.getCoastalDistanceMaintenanceModifier() * iChange);

			playerX.changeAssets(kTech.getAssetValue() * iChange);
			playerX.changeTechPower(kTech.getPowerValue() * iChange);
			...
			for (int iJ = 0; iJ < NUM_COMMERCE_TYPES; iJ++)
			{
				playerX.changeCommerceRateModifier((CommerceTypes)iJ, kTech.getCommerceModifier(iJ) * iChange);
			}
