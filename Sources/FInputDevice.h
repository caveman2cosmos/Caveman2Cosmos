...
class FInputDevice
{
  ...
  public:
	...
	struct InputValue
	{
		...
		InputValue(int iState = UNINIT, int px = -1, int py = -1)
			: m_iState(iState)
		{
			m_point.x = px;
			m_point.y = py;
	...
	struct InputEvent
	{
		...
		InputEvent()
			: m_uiModifiers(0), m_type(NONE) {}
		InputEvent(InputType it, InputValue iv, uint uiMod = 0)
			: m_type(it), m_val(iv), m_uiModifiers(uiMod) {}
